...
<% def last = '/wiki/' %>
<a class="breadcrumb-root breadcrumb" href="${last}">(Root)</a><span class="breadcrumb-sep">/</span><% if ( data.group ){ data.group.split('/').each { last = last + it + '/' %> <a class="breadcrumb" href="${last}">${it}</a><span class="breadcrumb-sep">/</span><% }} %><span class="breadcrumb">${data.localId}</span>
</div>
<% } %>
